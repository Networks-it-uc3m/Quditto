"""
The difference between "MLWE" problem and the "LWE" is that, in
"Module Learning with Errors", "Zq^n" is replaced by a certain
module "Rq^k", which is constructed by taking the k-fold Cartesian
product of a certain polynomial ring "Rq". Then, the secret in
this problem is an element "x" of module "Rq^k".

The ring "Rq" is the ring Zq[X]/((X^n)+1) of polynomials of the 
form "f = f0 + f1*X + f255*X^255" (where fj E Zq for all j),
with the ring operations defined by arithmetic modulo (X^n)+1.
"""
import random
...
class PolynomialRing:
    ...
    """
    This function returns the generator `x` of the
    polynomial ring.
    """
    def gen(self):
        return self([0, 1])
    ...
    """
    This function computes a random element of the
    polynomial ring with coefficients in the canonical
    range: "[0, q-1]".
    """
...
class Polynomial:
    ...
    """
    This function returns if polynomial is zero: f = 0.
    """
    def is_zero(self):
        return all(c == 0 for c in self.coeffs)

    """
    This function returns if polynomial is constant: f = c.
    """
    def is_constant(self):
        return all(c == 0 for c in self.coeffs[1:])

    """
    Helper function which right pads with zeros to allow
    polynomial construction as f = R([1,1,1]).
    """
    ...
    """
    This function reduces all coefficients modulo "q".
    """
    ...
    """
    This function adds two coefficients modulo "q".
    """
    def _add_mod_q(self, x, y): 
        return (x + y) % self.parent.q

    """
    This function subs two coefficients modulo "q".
    """
    def _sub_mod_q(self, x, y):
        return (x - y) % self.parent.q

    """
    This function is a naive implementation of polynomial
    multiplication suitible for all R_q = F_1[X]/(X^n + 1).
    """
    ...
    """
    This function returns -f, by negating all coefficients.
    """
    def __neg__(self):
